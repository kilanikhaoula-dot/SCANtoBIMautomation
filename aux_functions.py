...
def identify_slabs(points_xyz, points_rgb, bottom_floor_slab_thickness, top_floor_ceiling_thickness,
    ...
    for i in tqdm(range(n_steps), desc="Recherche des coordonnées Z des surfaces (dalles)"):
        z = z_min + i * z_step
        idx_selected_xyz = np.where((z <= points_xyz[:, 2]) & (points_xyz[:, 2] < (z + z_step)))[0]
        z_array.append(z)
        n_points_array.append(len(idx_selected_xyz))
        
    threshold = 0.5 * np.max(n_points_array)

    if plot_segmented_plane:
        # CORRECTION DU CRASH GRAPHIQUE
        max_n_points_array = np.max(n_points_array) 
    ...
    for interval in h_surf_candidates:
        if len(merged_candidates) == 0 or interval[0] > merged_candidates[-1][1]:
            ...
            merged_candidates[-1][1] = max(merged_candidates[-1][1], interval[1])

    horiz_surface_planes = []
    for i in tqdm(range(len(merged_candidates)), desc="Extraction des points pour les dalles"):
        horiz_surface_idx = np.where(
            (merged_candidates[i][0] < points_xyz[:, 2]) &
            (points_xyz[:, 2] < merged_candidates[i][1]))[0]
        if len(horiz_surface_idx) > 0:
            horiz_surface_planes.append(points_xyz[horiz_surface_idx])

    slabs = []
    os.makedirs("output_xyz", exist_ok=True)
    
    # NOUVELLE LOGIQUE : Indépendante des index pairs/impairs !
    # On traite chaque pic horizontal comme une dalle à part entière.
    for i in range(len(horiz_surface_planes)):
        print(f'Création de l\'empreinte pour la dalle n°{i + 1} sur {len(horiz_surface_planes)}.')
        
        # Le Z médian des points représente le haut de la dalle
        slab_top_z_coord = np.median(horiz_surface_planes[i][:, 2])
        
        # On assigne l'épaisseur (la dalle la plus basse prend l'épaisseur de sol, les autres celles de plafond/dalle sup)
        thickness = bottom_floor_slab_thickness if i == 0 else top_floor_ceiling_thickness
        slab_bottom_z_coord = slab_top_z_coord - thickness
        
        # Génération du contour géométrique 2D de la dalle
        x_coords, y_coords, polygon = create_hull_from_histogram(
            horiz_surface_planes[i], pc_resolution,
        ...
        print(f'Dalle n°{i + 1} enregistrée : bas (z) = {slab_bottom_z_coord:.3f} m, épaisseur = {thickness * 1000:.1f} mm')
...
def split_pointcloud_to_storeys(points_xyz, slabs):
    ...
    if len(slabs) < 2:
        print("⚠️ Moins de 2 dalles détectées, impossible de diviser en étages.")
        return []

    # Tri des dalles par coordonnée Z croissante (du bas vers le haut)
    slabs = sorted(slabs, key=lambda s: s['slab_bottom_z_coord'])
    
    print(f"Z points : {points_xyz[:, 2].min():.2f} → {points_xyz[:, 2].max():.2f}")
    
    # Marge de sécurité pour ne pas attraper les points du sol ou du plafond
    safety_margin = 0.05 
    
    for i in range(len(slabs) - 1):
        # On coupe juste en dessous de la dalle supérieure
        bottom_z_of_upper_slab = slabs[i + 1]['slab_bottom_z_coord'] - safety_margin
        
        # On coupe juste au-dessus de la dalle inférieure
        top_z_of_bottom_slab = slabs[i]['slab_bottom_z_coord'] + slabs[i]['thickness'] + safety_margin

        if top_z_of_bottom_slab >= bottom_z_of_upper_slab:
            print(f"⚠️ Étage {i} : limites incohérentes, veuillez réduire la safety_margin")
            continue

        print(f"Extraction Étage {i} : Z compris entre {top_z_of_bottom_slab:.2f} m et {bottom_z_of_upper_slab:.2f} m")

        segmented_pointcloud_idx = np.where(
            (points_xyz[:, 2] > top_z_of_bottom_slab) &
            (points_xyz[:, 2] < bottom_z_of_upper_slab)
        )[0]
        
        print(f" → {len(segmented_pointcloud_idx)} points de murs trouvés")
        if len(segmented_pointcloud_idx) > 0:
            segmented_pointclouds_3d.append(points_xyz[segmented_pointcloud_idx])
...
def save_coordinates_to_xyz(coordinates_list, base_filename):
    # CORRECTION : J'ai réparé la fin de cette fonction qui était coupée
