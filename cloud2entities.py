...
try:
    from aux_functions_improved import identify_slabs_improved, identify_openings_improved
    USE_IMPROVED_FUNCTIONS = True
    print("✅ Fonctions améliorées chargées (détection dalles/toits + ouvertures)")
except ImportError:
    print("⚠️ aux_functions_improved.py non trouvé, utilisation des fonctions originales")
    print("   Pour activer les améliorations, copiez aux_functions_improved.py dans ce dossier")
    USE_IMPROVED_FUNCTIONS = False
...
# === DÉTECTION AMÉLIORÉE DES DALLES ET TOITS ===
if USE_IMPROVED_FUNCTIONS:
    print("\n" + "="*70)
    print("🚀 UTILISATION DE LA DÉTECTION AMÉLIORÉE (RANSAC + Toits)")
    print("="*70)
    
    slabs, roofs, horizontal_surface_planes = identify_slabs_improved(
        points_xyz,
        bottom_floor_thickness=bfs_thickness,
        top_floor_thickness=tfs_thickness,
        z_step=0.15,
        pc_resolution=pc_resolution,
        ransac_threshold=0.05,  # 5cm pour précision
        detect_roof=True,
        plot_results=False
    )
    
    print(f"\n🎉 RÉSUMÉ DÉTECTION:")
    print(f"   ✅ {len(slabs)} dalle(s) détectée(s)")
    print(f"   ✅ {len(roofs)} toit(s) détecté(s)")
    print("="*70 + "\n")
else:
    # Fonction originale Cloud2BIM
    slabs, horizontal_surface_planes = identify_slabs(
        points_xyz, points_rgb, bfs_thickness,
        tfs_thickness, z_step=0.15,
        pc_resolution=pc_resolution,
        plot_segmented_plane=False
    )
    roofs = []  # Pas de toits avec la fonction originale
...
for i, storey_pointcloud in enumerate(point_cloud_storeys):
    ...
    for j in range(len(start_points)):
        ...
        # === DÉTECTION AMÉLIORÉE DES OUVERTURES ===
        if USE_IMPROVED_FUNCTIONS:
            (opening_widths, opening_heights, opening_types) = identify_openings_improved(
                wall_number=j + 1,
                wall_points=translated_filtered_rotated_wall_groups[j],
                wall_label=wall_labels[j],
                resolution=pc_resolution,
                grid_coefficient=grid_coefficient,
                min_opening_area=0.5,
                max_opening_area=10.0,
                min_opening_height=0.6,
            ...
            (opening_widths, opening_heights, opening_types) = identify_openings(
                j + 1, translated_filtered_rotated_wall_groups[j],
...
# ============================================================================
# EXPORT DES TOITS VERS IFC (NOUVEAU)
# ============================================================================
if USE_IMPROVED_FUNCTIONS and len(roofs) > 0:
    print("\n" + "="*70)
    print("🏠 CRÉATION IFC DES TOITS")
    print("="*70)
    
    for idx, roof in enumerate(roofs):
        print(f"\n🏠 Toit {idx+1}:")
        print(f"   Z: {roof['roof_z_coord']:.2f}m")
        print(f"   Pente: {roof['slope']:.1f}°")
        print(f"   Points: {len(roof['points']):,}")
        
        # Créer matériau pour toit
        roof_color_rgb = (0.6, 0.2, 0.2)  # Rouge foncé
        roof_material, roof_material_def = ifc_model.create_material_with_color(
            f"Roof_Material_{idx+1}",
            roof_color_rgb,
            transparency=0
        )
        
        # OPTION 1: Sauvegarder les points du toit en XYZ (temporaire)
        os.makedirs("output_xyz", exist_ok=True)
        roof_xyz_path = f'output_xyz/roof_{idx+1}.xyz'
        np.savetxt(roof_xyz_path, roof['points'], fmt='%.6f')
        print(f"   ✓ Points sauvegardés: {roof_xyz_path}")
        
        # OPTION 2: Créer comme IfcSlab au niveau le plus haut (temporaire)
        # NOTE: Pour un vrai IfcRoof, il faudrait:
        # - Créer IfcRoof au lieu de IfcSlab
        # - Gérer la pente avec IfcRoofType
        # - Créer la géométrie inclinée avec IfcExtrudedAreaSolid
        try:
            # Trouver le storey le plus haut
            highest_storey = storeys_ifc[-1]
            
            # Créer géométrie du toit comme dalle (temporaire)
            roof_polygon = roof['polygon']
            roof_z = roof['roof_z_coord']
            roof_thickness = 0.25  # 25cm
            
            roof_placement = ifc_model.wall_placement(roof_z)
            roof_closed_profile = ifc_model.slab_closed_profile_def(
                roof['polygon_x_coords'],
                roof['polygon_y_coords']
            )
            roof_extrusion = ifc_model.slab_extrusion(roof_closed_profile, roof_thickness)
            roof_representation = ifc_model.slab_representation(roof_extrusion)
            roof_product_definition = ifc_model.product_definition_shape_slab(roof_representation)
            
            # Créer le toit comme dalle (temporaire - à remplacer par IfcRoof)
            roof_slab = ifc_model.create_slab(
                roof_placement,
                roof_product_definition,
                f"Roof_{idx+1}",
                "ROOF"  # Type de dalle
            )
            
            # Assigner au dernier étage
            ifc_model.assign_product_to_storey(roof_slab, highest_storey)
            ifc_model.assign_material(roof_slab, roof_material)
            
            print(f"   ✓ Toit exporté en IFC (comme IfcSlab temporairement)")
            print(f"   ⚠️ TODO: Implémenter IfcRoof pour gérer la pente correctement")
            
        except Exception as e:
            print(f"   ⚠️ Erreur export IFC toit: {e}")
            print(f"   → Les points sont sauvegardés en XYZ")
    
    print(f"\n✅ {len(roofs)} toit(s) traité(s)")
    print("="*70 + "\n")
...
if USE_IMPROVED_FUNCTIONS:
    total_openings = sum(1 for wall in walls for _ in all_openings if _['opening_wall_id'] == wall['wall_id'])
    summary_msg = f'\n{"="*70}\n'
    summary_msg += f'IFC model saved to {ifc_output_file}\n'
    summary_msg += f'{"="*70}\n'
    summary_msg += f'RÉSUMÉ FINAL - DÉTECTION AMÉLIORÉE:\n'
    summary_msg += f'  • Dalles: {len(slabs)}\n'
    summary_msg += f'  • Toits: {len(roofs)}\n'
    summary_msg += f'  • Murs: {len(walls)}\n'
    summary_msg += f'  • Ouvertures: {len(all_openings)}\n'
    summary_msg += f'  • Zones: {len(zones)}\n'
    summary_msg += f'{"="*70}'
    last_time = log(summary_msg, last_time, log_filename)
else:
    last_time = log(f'\nIFC model saved to {ifc_output_file}.', last_time, log_filename)